...
async function Page() {
  ...
  return (
    <div className={"bg-card"}>
      <Card className={"rounded-none border-none px-4 py-7 w-full max-w-7xl mx-auto"}>
        <CardHeader>
      ...
      <Card className={"rounded-none border-none px-4 py-7 w-full max-w-7xl mx-auto"}>
        <CardHeader>
          <CardTitle><h2 className={"text-3xl"}>Goal Progress</h2></CardTitle>
        </CardHeader>
        <CardContent className={"gap-4 flex flex-wrap"}>
          <Card>
      ...
      <Card className={"border-none px-4 py-7 w-full mx-auto rounded-none max-w-7xl"}>
        <CardHeader>
          <CardTitle><h4 className={"text-3xl"}>Activity Heatmap</h4></CardTitle>
        </CardHeader>
        <CardContent className={"grid grid-cols-20 gap-1 justify-center w-max border py-6 rounded-[inherit]"}>
          {Array.from({ length: 100}) .map((value, index) => (<div key={index} className={"bg-green-300 w-4 h-4 rounded"}></div>))}
        </CardContent>
      </Card>
    </div>
  );
